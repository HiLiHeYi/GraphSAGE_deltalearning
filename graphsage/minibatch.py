...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx,
        ...
        if not n2v_retrain:
            ...
            self.val_edges = [e for e in G.edges() if G[e[0]]
                              [e[1]]['train_removed']]
        else:
            if fixed_n2v:
        ...
        print(len([n for n in G.nodes() if G.node[n]
                   ['test'] or G.node[n]['val']]), 'test nodes')
        self.val_set_size = len(self.val_edges)

    def _n2v_prune(self, edges):
        def is_val(n): return self.G.node[n]["val"] or self.G.node[n]["test"]
...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx,
        ...
        if not self.ppr_only:
            self.val_nodes = [
                n for n in self.G.nodes() if self.G.node[n]['val']]
            self.test_nodes = [
                n for n in self.G.nodes() if self.G.node[n]['test']]

            self.no_train_nodes_set = set(self.val_nodes + self.test_nodes)
            self.train_nodes = set(G.nodes()).difference(
                self.no_train_nodes_set)
        # don't train on nodes that only have edges to test set
        else:
            ## setting nodes to all sets (we do not consider val/test set, but graphsage needs it)
            self.val_nodes = self.G.nodes()
            self.test_nodes = self.G.nodes()
            self.no_train_nodes_set = {}
            self.train_nodes = self.G.nodes()
    ...
    ### select 1-hop neighbors (25 nodes)
    ### if less then 25 nodes repeating until reaching 25 nodes
    #  
    # def construct_adj(self):
    #     adj = len(self.id2idx)*np.ones((len(self.id2idx)+1, self.max_degree))
    #     deg = np.zeros((len(self.id2idx),))
    ...
    #             continue
    #         temp = neighbors
    #         while len(neighbors) < self.max_degree:
    #             neighbors = np.append(neighbors, temp)
    ...
    def node_val_feed_dict(self, size=None, test=False):
        ...
        ## Debug! to change batch 
        # ret_val = self.batch_feed_dict([0, 1, 2, 3])
    ...
    def incremental_node_val_feed_dict(self, size, iter_num, test=False):
        ...
        # Debug! to change batch
        # val_node_subset = self.val_nodes
    ...
    def next_minibatch_feed_dict(self, batch=None):
        start_idx = self.batch_num * self.batch_size
        self.batch_num += 1
        end_idx = min(start_idx + self.batch_size, len(self.train_nodes))
        batch_nodes = self.train_nodes[start_idx: end_idx]

        # Debug to return a custom batch
